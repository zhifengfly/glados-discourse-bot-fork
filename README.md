...
Discourse 看着统一，实际各有各的幺蛾子：

- **URL 路径**：大部分用 `/t/{id}`，NodeSeek 用 `/n/topic/{id}`（没错就是不一样）
- **Cloudflare**：linux.do 放行 API（`/latest.json`）但拦页面请求，cron 读帖必须 best-effort
- **CSRF**：所有 Discourse 登录 cookie 都带 `_t`，直接用就行，不要绕路抓页面
- **30 秒限制**：webhook 回调里所有操作加起来不能超过 30 秒，手动读跑 5 帖 3-7 秒/帖刚好卡线
